	...
	for (i = 0; i < ect_gen_param_header->num_of_table; ++i) {
		...
		for (j = 0; j < table->num_of_row; ++j) {
			...
			for (k = 0; k < table->num_of_col; ++k) {
				//if(j * table->num_of_col + k==2184000)  //test
				//	seq_printf(s, "%u ", table->parameter[2288000]);
				//else
